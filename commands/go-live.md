...
3. **Config in sync + i18n:** if `webapp.config.json` exists, run `ro app config push -e production --dry-run` and show the user any pending New/Update/Delete delta. As part of this, check the metadata's i18n: `title`/`description` should carry `localization` entries for every language the app's audience uses — show the user the exact strings per language and get their confirmation before pushing. Going up a rung with untranslated or unreviewed launcher copy is a common miss: the new rung's viewers may not read the default language. Resolve the delta (`ro app config push -e production -y`, or edit the file and re-check) BEFORE sharing, so the live app reflects the intended, confirmed config.
