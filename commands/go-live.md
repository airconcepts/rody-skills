...
3. **Config in sync + launcher metadata:** if `webapp.config.json` exists, run `ro app config push -e production --dry-run` and show the user any pending New/Update/Delete delta. As part of this, check what the new rung's viewers will actually see: **`coverImg` is set and shows the intended image** (no cover = an empty card in front of the new audience; fix via `ro app assets push <img> --dest images -e production --json` → put the URL in `coverImg`), and `title`/`description` carry `localization` entries for every language the app's audience uses — show the user the cover and the exact strings per language and get their confirmation before pushing. Going up a rung with a missing cover or unreviewed copy is a common miss. Resolve the delta (`ro app config push -e production -y`, or edit the file and re-check) BEFORE sharing, so the live app reflects the intended, confirmed config.
